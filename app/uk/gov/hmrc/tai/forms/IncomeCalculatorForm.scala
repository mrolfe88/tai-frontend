...
object TaxablePayslipForm{
  ...
  def createForm(netSalary: Option[String] = None, payPeriod:Option[String] = None, payPeriodInDays: Option[String] = None)
    ...
    val x = netSalary match {
      case salary => messages("tai.taxablePayslip.error.form.incomes.radioButton.mandatory")
      case _ => TaxablePayPeriod.errorMessage(payPeriod, payPeriodInDays)
    }
