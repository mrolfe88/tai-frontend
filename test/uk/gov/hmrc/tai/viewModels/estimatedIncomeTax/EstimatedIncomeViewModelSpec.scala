...
class EstimatedIncomeViewModelSpec extends PlaySpec with FakeTaiPlayApplication with I18nSupport with TaxRegionConstants with BandTypesConstants {
  ...
  "merge Zero bands" must {
      ...
      "return a merged list when two individual lists are supplied " in {
        val individualList = List(Band("TaxFree",50,1000,0,"ZeroBand"),
          Band("TaxFree",50,1000,0,"ZeroBand"))
  ...
  "getUpperBand" must {
    ...
    "deduct personal allowance from tax-free allowances to get the upper-band if user is in higher rate" in {
        ...
        val upperBand = BandedGraph.getUpperBand(taxBands = taxBands, personalAllowance = Some(5000), taxFreeAllowanceBandSum = 5000)
  ...
  "calculate bar percentage" must {
    ...
    "return valid percentage for first income when two bands has passed" in {
      ...
      val percentage = BandedGraph.calcBarPercentage(3200, taxBands, taxFreeAllowanceBandSum = 3200, totalTaxBandIncome = 19200,
        taxViewType = ComplexTaxView)
    ...
    "return valid percentage for second income when two bands has passed" in {
      ...
      val percentage = BandedGraph.calcBarPercentage(16000, taxBands, taxFreeAllowanceBandSum = 3200,totalTaxBandIncome = 19200,
        taxViewType = ComplexTaxView)
    ...
    "return value till two decimal" in {
      ...
      val percentage = BandedGraph.calcBarPercentage(15000, taxBand, taxFreeAllowanceBandSum = 4000, totalTaxBandIncome = 23000,
        taxViewType = ComplexTaxView)
